...
def get_image_transform_resize_crop(
        input_res: Tuple[int,int], 
        output_resize_res: Tuple[int,int] | None, 
        output_crop_res: Tuple[int,int] | None, 
        out_resolutions_crop_random_ratio: Tuple[float,float] | None = None,
        bgr_to_rgb: bool=False,
        is_depth: bool=False,
        is_pcd: bool=False,
        crop_bias: Tuple[int,int]=(0,0)):

    if output_resize_res is None or output_crop_res is None:
        def identify_transform(input_res: Tuple[int,int], output_resize_res: Tuple[int,int] | None, output_crop_res: Tuple[int,int] | None):
            return img
        return identify_transform
    ...
    def transform(img: np.ndarray):
        ...
        if out_resolutions_crop_random_ratio is None:
            random_scale_w, random_scale_h = 1.0, 1.0
        elif out_resolutions_crop_random_ratio[0] == 1.0 and out_resolutions_crop_random_ratio[1] == 1.0:
            random_scale_w, random_scale_h = 1.0, 1.0
        else:
            random_scale_w = np.random.uniform(out_resolutions_crop_random_ratio[0], out_resolutions_crop_random_ratio[1])
            random_scale_h = np.random.uniform(out_resolutions_crop_random_ratio[0], out_resolutions_crop_random_ratio[1])
        th = (img.shape[0] - int(output_crop_res[1] * random_scale_h)) // 2 + crop_bias[1]
        tw = (img.shape[1] - int(output_crop_res[0] * random_scale_w)) // 2 + crop_bias[0]
        img = img[th:th+int(output_crop_res[1] * random_scale_h), tw:tw+int(output_crop_res[0] * random_scale_w)]
        # print(f"img.shape: {img.shape}, random_scale_w: {random_scale_w}, random_scale_h: {random_scale_h}")
        return img
