        ...
        if inputs_embeds[1] is None:
            ...
            def compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond):
                ...
                for i, hidden_states in enumerate(inputs_embeds):
                    ...
                    # Though the hidden_states dimension may be different, the head_dim & num_head is set to the same value for all layers (src/openpi/models/gemma.py)
                ...
                query_states = torch.cat(query_states, dim=2)   # (B, num_heads, seq_len, head_dim)
                key_states = torch.cat(key_states, dim=2)
                value_states = torch.cat(value_states, dim=2)

                dummy_tensor = torch.zeros(
                    query_states.shape[0],      # (B)
                    query_states.shape[2],      # seq_len
                ...
                att_output = att_output.reshape(batch_size, -1, 1 * 8 * head_dim)     # num_head = 8 is set in the config
