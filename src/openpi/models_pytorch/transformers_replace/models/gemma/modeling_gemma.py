...
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x, cond=None, active_tail_tokens: Optional[int] = None):
        ...
        if cond.ndim not in (2, 3):
            raise ValueError(f"Unsupported cond rank {cond.ndim}, expected 2 or 3")

        modulation = self.dense(cond)
        if cond.ndim == 2 and x.ndim == 3:
            modulation = modulation.unsqueeze(1)

        if active_tail_tokens is not None:
            if x.ndim != 3:
                raise ValueError("active_tail_tokens can only be used for sequence inputs (x.ndim == 3)")
            if active_tail_tokens < 0:
                raise ValueError(f"active_tail_tokens must be >= 0, got {active_tail_tokens}")

            if modulation.ndim == 2:
                modulation = modulation.unsqueeze(1)
            if modulation.shape[1] == 1:
                modulation = modulation.expand(-1, x.shape[1], -1)
            elif modulation.shape[1] != x.shape[1]:
                raise ValueError(
                    f"modulation sequence length ({modulation.shape[1]}) does not match x ({x.shape[1]})"
                )

            if active_tail_tokens == 0:
                modulation = torch.zeros_like(modulation)
            elif active_tail_tokens < x.shape[1]:
                active_mask = torch.zeros(
                    modulation.shape[0], modulation.shape[1], 1, device=modulation.device, dtype=modulation.dtype
                )
                active_mask[:, -active_tail_tokens:, :] = 1
                modulation = modulation * active_mask
...
class GemmaDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        adarms_active_tail_tokens: Optional[int] = None,
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> tuple[torch.FloatTensor, Optional[tuple[torch.FloatTensor, torch.FloatTensor]]]:
        residual = hidden_states
        hidden_states, gate = self.input_layernorm(
            hidden_states, adarms_cond, active_tail_tokens=adarms_active_tail_tokens
        )
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        adarms_active_tail_tokens (`int`, *optional*):
            If set, ADARMS modulation is only applied on the last `adarms_active_tail_tokens` sequence tokens.
            Use `0` to disable ADARMS modulation on all sequence tokens.
        """
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            layer_outputs = decoder_layer(
                ...
                adarms_active_tail_tokens=adarms_active_tail_tokens,
                **kwargs,
            )
        ...
        hidden_states, _ = self.norm(hidden_states, adarms_cond, active_tail_tokens=adarms_active_tail_tokens)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        adarms_cond (`torch.Tensor` of shape `(batch_size, cond_dim)`, *optional*):
            Condition for ADARMS.
        adarms_active_tail_tokens (`int`, *optional*):
            Passed to the decoder to limit ADARMS modulation to the last
            `adarms_active_tail_tokens` sequence tokens.
